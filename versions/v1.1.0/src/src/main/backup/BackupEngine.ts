...
export class BackupEngine extends EventEmitter {
  ...
  createTask(config: TaskConfig): BackupTask {
    ...
    // Date folder from shootingDate (YYYY-MM-DD → YYYYMMDD); fallback to today
    ...
    const task: BackupTask = {
      ...
      shootingDateFolder: shootingDate,
      status: 'pending',
      totalFiles: 0,
      completedFiles: 0,
      totalBytes: 0,
  ...
  async startTask(taskId: string): Promise<void> {
    ...
    try {
      ...
      // Build destination roots: dest/date/device/volume for each device
      // Structure: dest.path / shootingDateFolder / deviceName / volumeName
      const dateFolder = task.shootingDateFolder ?? ''
      for (const dest of task.destinations) {
        ...
        for (const device of deviceSubfolders) {
          const deviceRoot = device
            ? path.join(dest.path, dateFolder, device, volumeName)
            : path.join(dest.path, dateFolder, volumeName)
          await fs.promises.mkdir(deviceRoot, { recursive: true })
        }
        // Store just the date root for resolvedPath reference
        dest.resolvedPath = path.join(dest.path, dateFolder)
      }
  ...
  private async copyFileToAllDestinationsParallel(
    ...
    const destResults = await Promise.all(
      task.destinations.map(async (dest) => {
        const dateRoot = dest.resolvedPath ?? dest.path
        const volumeName = task.namingTemplate
        // Structure: dest/date/device/volume/relativePath
        const deviceSubfolders = task.devices.length > 0 ? task.devices : ['']
        const results = await Promise.all(
          deviceSubfolders.map(async (device) => {
            const deviceRoot = device
              ? path.join(dateRoot, device, volumeName)
              : path.join(dateRoot, volumeName)
